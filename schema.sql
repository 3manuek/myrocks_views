...
SELECT DDL.CF  as "CF",MAP.SST_NAME,group_concat(concat(DDL.TABLE_SCHEMA, '.', DDL.TABLE_NAME, '->', ifnull(DDL.INDEX_NAME, 'default'))) as "INDEX",
     sum(MAP.NUM_ROWS) as "ROWS", sum(MAP.DATA_SIZE) as "DATA_SIZE",
    sum(MAP.ENTRY_DELETES) as "DELETES", sum(MAP.ENTRY_SINGLEDELETES) as "SINGLE_DELS", sum(MAP.ENTRY_MERGES) as "MERGES", 
    sum(MAP.ENTRY_OTHERS) as "OTHERS"
FROM information_schema.ROCKSDB_DDL DDL
  JOIN information_schema.ROCKSDB_INDEX_FILE_MAP MAP
  ON (MAP.INDEX_NUMBER = DDL.INDEX_NUMBER)
GROUP BY 1, 2 
